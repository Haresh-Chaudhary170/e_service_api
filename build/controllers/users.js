...
let UserController = class UserController {
    ...
    uploadKycAndCertifications(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const kycDocument = yield prisma.document.create({
                    data: {
                        providerId,
                        type,
                        name,
                ...
                if (certifications && Array.isArray(certifications)) {
                    for (let i = 0; i < certifications.length; i++) {
                        ...
                        if (!file)
                            continue;
                        const certDoc = yield prisma.certification.create({
                            data: {
                                providerId,
                                title: cert.title,
    ...
    uploadAddress(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const existingUser = yield prisma.user.findUnique({ where: { id: req.user.id } });
            if (!existingUser) {
                return res.status(404).json({ error: "User not found" });
            }
            try {
                ...
                if (existingAddress) {
                    ...
                    address = yield prisma.address.update({
    ...
    getProviderss(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const provider = yield prisma.serviceProvider.findMany({
                    include: {
                        user: true,
                        category: true,
                        serviceAreas: true,
    ...
    getProviderById(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.user;
            try {
                const provider = yield prisma.serviceProvider.findFirst({ where: { userId: id } });
                if (!provider) {
                    return res.status(404).json({ error: "Provider not found" });
                }
    ...
    updateProvider(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const existingProvider = yield prisma.serviceProvider.findFirst({ where: { userId: id } });
                if (!existingProvider) {
                    return res.status(404).json({ error: "Provider not found" });
                }
                // Update the provider in the database
                const provider = yield prisma.serviceProvider.update({
