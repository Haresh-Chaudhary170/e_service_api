    ...
    login(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const isPasswordValid = (user === null || user === void 0 ? void 0 : user.password) ? yield bcryptjs_1.default.compare(password, user.password) : false;
                if (!isPasswordValid) {
                ...
                if (user.role === "CUSTOMER") {
                    const customer = yield prisma.customer.findUnique({ where: { userId: user.id } });
                    customerId = customer === null || customer === void 0 ? void 0 : customer.id;
                }
                else if (user.role === "SERVICE_PROVIDER") {
                    const provider = yield prisma.serviceProvider.findUnique({ where: { userId: user.id } });
