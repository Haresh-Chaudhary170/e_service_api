...
let BookingController = class BookingController {
    ...
    async getBookingsByCustomerId(req, res, next) {
        const { id } = req.params;
        try {
            const bookings = await prisma.booking.findMany({
                where: { customerId: id },
    ...
    async getBookingsByProviderId(req, res, next) {
    ...
    async createBooking(req, res, next) {
        ...
        try {
            ...
            await (0, googleCalendar_1.createGoogleCalendarEvent)(userGoogleCredentials.googleAccessToken, `Booking with ${provider.businessName}`, `Booking ID: ${booking.id}\nCustomer: ${user.firstName} ${user.lastName}\nServices: ${cartItems.map(item => item.serviceId).join(", ")}`, startDateTime.toISOString(), endDateTime.toISOString());
            res.status(200).json(booking);
        }
        catch (error) {
            console.error("Error creating booking:", error);
