...
class AuthController {
    ...
    @Route('post', '/auth/change-password', checkRole(['CUSTOMER', 'SERVICE_PROVIDER']))
    @Validate(changeValidationSchema) // Validation on the request body
    async changePassword(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            // Validate the password
            const isPasswordValid = user?.password ? await bcrypt.compare(oldPassword, user.password) : false;

            if (!isPasswordValid) {
                return res.status(401).json({ error: "Old password is incorret!" });
            }
            ...
            return res.json({
                ...
                user: { firstName: updates.firstName, lastName: updates.lastName, email: updates.email, role: updates.role },
            });
